import * as s from '../service/noticias.service.js';
export const getAll = async (req,res,next)=>{try{res.json({success:true,data:await s.listar()});}catch(e){next(e);}};
export const getById = async (req,res,next)=>{try{res.json({success:true,data:await s.obtener(+req.params.id)});}catch(e){next(e);}};
